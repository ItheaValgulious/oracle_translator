...
- `few_shot_examples`
- `candidate_model_socket_from_source_meta`

它们的用途是:

- `few_shot_examples` 给你看这一批语料里已经认可的映射风格
- `candidate_model_socket_from_source_meta` 不是让你机械抄写, 而是提醒你当前这条样本的人类来源语义预期

如果咒语文本和这个候选结构没有明显冲突, 你应优先输出与它一致的结果.
只有在文本本身明确反驳这个候选结构时, 才允许偏离.
