...
from .model_socket_schema_v2 import (
...
STYLE_RECIPES = [
    StyleRecipe("semi_classical_balanced", "半文半白平衡", 0.72, "像认真施法的玩家, 略带古意, 但不过典.", "不要变成纯文言."),
    StyleRecipe("semi_classical_literary", "半文半白偏文学", 0.82, "半文半白, 有修辞, 但仍可理解.", "不要堆生造词."),
    StyleRecipe("ritual_prayer", "仪式祈请", 0.90, "像誓辞或祈请, 仪式感强.", "不要太生活化."),
    StyleRecipe("literary_soft", "文学描写偏柔", 0.76, "文学性高, 有画面, 更柔和.", "不要写成旁白."),
    StyleRecipe("literary_sharp", "文学描写偏锋利", 0.74, "文学性高, 但更有推进感.", "不要拖太长."),
    StyleRecipe("detail_literary", "细节描写文学", 0.88, "大量细节和修辞, 颜色, 质地, 动势都可以写.", "不要写成长说明文."),
    StyleRecipe("everyday_clean", "生活化克制", 0.38, "像玩家自然说出来的话, 清楚, 完整.", "不要彻底失去施法感."),
    StyleRecipe("battle_colloquial", "口语战斗", 0.18, "像战斗里急促喊出来的话.", "不要写成残句."),
    StyleRecipe("colloquial_with_lift", "口语里带一点文气", 0.28, "主体口语, 但更凝练.", "不要突然切回古风."),
    StyleRecipe("modern_reference_light", "轻度现实参照", 0.34, "允许熟悉现实物象进入句子.", "不要变成科技说明."),
    StyleRecipe("modern_reference_heavy", "强现实参照", 0.20, "明显借用现实世界物象.", "不要写成长解释句."),
    StyleRecipe("hacky_clever", "人类钻空子", 0.16, "像玩家在试探系统边界, 用化学物质, 漏洞思维或极端规则描述.", "不要写成参数串."),
    StyleRecipe("classical_direct", "古意但直接", 0.78, "有古意, 但说法直接.", "不要写得太浮艳."),
    StyleRecipe("high_ritual_high_literary", "高仪式高文学", 0.96, "非常正式, 庄重, 文学, 仪式化.", "不要变成纯谜语."),
]
...
MODEL_SOCKET_BLUEPRINTS = {
    ...
    "paper_flood": {"material_template": "wood", "reaction_template": "none", "release_template": "appear", "motion_template": "flow", "motion_direction": "forward", "origin": "self", "target": "none"},
...
def _build_spell_generation_user_prompt(
    *,
    ...
    model_socket = _model_socket_from_blueprint(motif.motif_id, recipe.powerness)
    return (
        f"{template}\n\n"
        f"recipe_name:\n{recipe.name}\n\n"
        f"powerness_target:\n{json.dumps(recipe.powerness, ensure_ascii=False)}\n\n"
        f"recipe_guidance:\n{recipe.guidance}\n\n"
        f"content_motif:\n{motif.prompt_hint}\n\n"
        f"avoid_patterns:\n{recipe.avoid}\n\n"
...
def _model_socket_from_blueprint(blueprint_id: str, powerness: float) -> dict[str, Any]:
    ...
    model_socket = {
        ...
        "expression": {"powerness": powerness},
    }
...
def _fallback_model_socket_from_source(source: dict[str, Any]) -> dict[str, Any]:
    ...
    powerness = meta.get("powerness_target")
    if motif_id is None or powerness is None:
        raise ValueError("source row does not have motif_id/powerness_target for fallback")
    if motif_id not in MODEL_SOCKET_BLUEPRINTS:
        if motif_id == "wild_fire":
            motif_id = "holy_fire"
        else:
            raise KeyError(motif_id)
    return _model_socket_from_blueprint(motif_id, float(powerness))
...
def _fallback_spell_from_model_socket(model_socket: dict[str, Any]) -> str:
    ...
    powerness = float(model_socket["expression"]["powerness"])
    ...
    if powerness >= 0.65:
        return f"{noun}{release_phrase}, {direction_phrase}."
    if powerness >= 0.35:
        return f"把这股{noun}放出去, {direction_phrase}."
...
def write_model_socket_seed_samples(*, output_path: str | Path, target_count: int, rng_seed: int = 23) -> list[dict[str, Any]]:
    ...
    while len(rows) < target_count:
        ...
        row = {
            "id": _stable_id("model_socket_seed_v2", motif_id + f"::{recipe.recipe_id}::{len(rows)}"),
            "model_socket": _model_socket_from_blueprint(motif_id, recipe.powerness),
            "meta": {
                "source": "model_socket_seed_v2",
                "recipe_id": recipe.recipe_id,
                "recipe_name": recipe.name,
                "powerness_target": recipe.powerness,
                "motif_id": motif_id,
            },
        }
...
def generate_spells(
    *,
    ...
    template = load_prompt_template("spell_generation_prompt_v2.md")
        ...
        while len(rows) < target_count:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    row = {
                        "id": _stable_id("spell_v2", text),
                        "text": text,
                        "meta": {
                            "source": "spell_generation_v2",
                            "recipe_id": recipe.recipe_id,
                            "recipe_name": recipe.name,
                            "powerness_target": recipe.powerness,
...
def translate_spells_to_json(
    *,
    ...
    template = load_prompt_template("spell_to_json_prompt_v2.md")
        ...
        for source in source_rows:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    validate_model_socket(model_socket)
                    row = {
                        "id": _stable_id("spell_to_json_v2", source["id"] + "::" + source["text"]),
                        "text": source["text"],
                        "model_socket": model_socket,
                        "meta": {
                            "source": "spell_to_json_v2",
                ...
                try:
                    ...
                    row = {
                        ...
                        "model_socket": model_socket,
                        "meta": {
                            "source": "spell_to_json_v2_fallback",
                            "source_spell_id": source["id"],
                            "source_meta": source.get("meta", {}),
                            "model_name": model_name,
                        },
...
def json_rows_to_spells(
    *,
    ...
    existing_texts = {_normalized_text(row["text"]) for row in rows}
    template = load_prompt_template("json_to_spell_prompt_v2.md")
    ...
    try:
        for source in source_rows:
            ...
            validate_model_socket(model_socket)
            recipe_powerness = source.get("meta", {}).get("powerness_target")
            recipe = StyleRecipe(
                source.get("meta", {}).get("recipe_id", "external"),
                source.get("meta", {}).get("recipe_name", "external"),
                float(recipe_powerness or model_socket["expression"]["powerness"]),
                "按给定 powerness 生成.",
                "不要重复原句.",
            )
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if text in existing_texts:
                        raise ValueError(f"duplicate text: {text}")
                    row = {
                        "id": _stable_id("json_to_spell_v2", source["id"] + "::" + text),
                        "text": text,
                        "model_socket": model_socket,
                        "meta": {
                            "source": "json_to_spell_v2",
                ...
                try:
                    ...
                    if text not in existing_texts:
                        row = {
                            ...
                            "model_socket": model_socket,
                            "meta": {
                                "source": "json_to_spell_v2_fallback",
                                "source_model_socket_id": source["id"],
                                "source_meta": source.get("meta", {}),
                                "model_name": model_name,
                            },
