...
- GPU 里的气体会保留更强的布朗运动,避免只被浮力和压差单向拉走; 高温会额外放大气体随机扰动的温度项。
- GPU 里的气体浮力当前按温度修正后的等效密度计算,温度越高越轻。
- GPU 里的 `empty` 热空气当前也会在 `motion` 里参与交换,但只在温度明显偏离环境或已有残余速度时才会主动移动; 其温度相关布朗增益更高。
...
- GPU 的 `blocked_impulse` 成功移动后也会衰减并夹到上限,避免横向残余过大导致浮空粒子。
...
- `thermal` 阶段对液体-水蒸气相界面施加额外传热倍率,让被水包裹的蒸汽团能更快冷却凝结,但不会放大 `fire`、毒气或酸雾对水的直接加热。
- `thermal` 阶段对水蒸气团内部也施加额外传热倍率,让蒸汽团中心热量能传到边界。
