...
- `W/A/S/D` 和方向键支持按住后连续平移相机,并在需要时触发活动窗分页平移。
- 相机连续移动当前会对单帧用于移动的有效 `dt` 做上限裁剪,避免掉帧后单帧位移暴涨,导致一帧内连续触发多次分页。
...
- overlay 当前会实时显示 world 尺寸、活动窗尺寸、viewport 尺寸、相机位置、活动窗原点、分页统计、pending writeback 压力、分页分阶段耗时,以及 `camera / step / io / upload / tick / draw` 的整帧阶段耗时,并额外显示本帧相机实际平移了多少 cells,便于区分切页卡顿和掉帧后位移追赶。
