...
- 求解器当前只按 `matter_state` 分成 `solid/liquid/gas` 三条大分支,不再额外使用 `motion_mode` 之类的离散运动类型。
- `solid`
  - 是否几乎不动、是否受风拖拽、是否在正下受阻时只试左右下对角,都由 `mobility / pressure_response / wind_coupling / downward_blocked_diagonal_fallback` 等参数决定
  - 承重静态固体当前通过参数组合保持不动,而不是走独立类型
- `liquid`
  - 使用极小的温度相关布朗运动
  ...
  - 当首选离散方向受阻时,默认仍会尝试夹角最近的其他可交换方向,但这层 fallback 现在带运行时开关
  - fallback 只会考虑与当前期望方向夹角小于等于 `directional_fallback_angle_limit_degrees` 的候选; 当前默认上限为 `45°`,因此正好 `45°` 的对角候选会被允许,但 `90°` 的纯侧向候选仍不会被当作 fallback
  - 关闭 fallback 后只尝试首选方向,便于让狭口出流保持更窄的水柱
- `gas`
  - 按温度修正后的等效密度决定上浮或下沉
...
- `empty`
  - 当前作为空气格参与对流
  - 环境空气当前不再默认是全图同温,而是围绕一条“近地略暖,高空略冷”的弱背景温度梯度活动
  - 默认新建出来的空空气当前就会按所在高度初始化到这条背景层结,而不是一律从 `20°C` 起步
  - `thermal` 会让空空气缓慢回归这条背景梯度,即使没有火焰也会形成更像现实的弱层结
  - 只有在温度明显偏离环境或已有残余速度时才会主动移动
  - 热空气使用更高的温度相关布朗增益,让热空气团更活跃
  - 空空气当前也会真正吃到由自身温差形成的局部 pressure force,让热羽流会卷动周围空气,而不只是热团自己上浮
  - 空空气包不会主动去替换显式气体包,显式气体自己负责穿过环境空气传播
...
它有指数衰减; 成功移动后留下的轴向残余也要衰减并限制上限,避免横向残余过大把粒子长期浮空。

当前离散选方向和冲突决策还额外加入了轻量去偏置规则:

- 当左右两个对角方向与当前意图同分时,会按 `step_id + x + y` 交替侧偏打破平局,避免长期固定朝单侧传播成 `45°` 空线
- 当多个 source 同时抢占同一目标格时,CPU/GPU 都会按随 `step_id` 交替镜像的顺序决胜,减少固定左上到右下的求解倾向
...
当前主 `motion` 还会先跑一次“非气态物质”交换,再跑一次“空气/气体”交换:

- 这样空气仍会形成对流和风
- 但不会因为空气本帧也在动,就提前占掉本该让沙子或液体落进去的空格
...
- 水家族相变带滞后:
  - 水要降到冰点以下一小段才结冰
  - 冰要升到冰点以上一小段才融化
  - 水蒸气在普通空气中要冷到更低温才凝结,但接触液体时仍会通过相界面传热更快冷却
  - 水蒸气团内部会有额外热交换,避免大团蒸汽中心近似绝热
...
- overlay 里会把方向 fallback 的角度上限明确显示成 `<= angle`
- 鼠标绘制/擦除
...
- 开启 `downward_blocked_diagonal_fallback` 的非承重固体变体当前只靠主 `motion` 中“正下受阻时向左下/右下释放”的规则来摊成坡,不再追加额外粉体 pass
- `acid` 当前在成功腐蚀承重目标后会自耗
- `poison` 当前主要是受热挥发成更易扩散的毒气,本轮不主打地形腐蚀
- 温度当前可以通过空气、`platform/fixpoint` 等固体和流体一起传导
- 空环境当前还会缓慢回归弱背景层结,让地面附近空气略暖、高空略冷,避免只有火焰才会制造任何可见温度结构
- 默认创建、清空或擦除得到的 `empty` 空空气当前就会直接落在本地环境温度上,避免用户一笔擦出整条 `20°C` 假温带
- 液体和水蒸气相邻时,热传导会有相界面加速项,用于让水包裹的蒸汽团能在可感知时间内冷却和凝结; `fire`、毒气和酸雾不使用这个倍率
- 水蒸气团内部也有额外热交换倍率,避免成团蒸汽中心长期保温
- 温度是 cell 的动态状态,运动或交换时随 cell 一起移动
- 物质离开某格时,留下的 `empty` 空气仍保留被加热后的温度,尤其 `fire` 移走或熄灭不会把原位置重置成默认空气温度
- 热空气当前除了导热,还会通过空气格交换向上漂移,并能通过局部 pressure 差卷动周围空气形成更复杂的对流风
